...
class ResultHandler:
    ...
    def _format_segment(self, r: dict, idx: int, store_results: bool = True) -> str:
        ...
        self.results["Value"].append((
            f"\n    • V₀ (baseline)        : {v0:.2f}\n"
            f"    • Vₚ (peak)            : {vp:.2f}\n"
            f"    • ΔV (amplitude)       : {dv:.2f}\n"
            f"    • V₉₀ (90 %)           : {v90:.2f}\n"
            f"    • V₁₀ (10 %)           : {v10:.2f}\n"
            f"    • [K⁺]₀ (baseline)     : {k0:.2f}\n"
            f"    • [K⁺]ₚ (peak)         : {kp:.2f}\n"
            f"    • Δ[K⁺] (amplitude)    : {dk:.2f}\n"
            f"    • V₉₀ (mM)             : {k90:.2f}\n"
            f"    • V₁₀ (mM)             : {k10:.2f}\n"
            f"    • τ (Tau)              : {tau_s_str}   ({tau_ms_str})\n"
            f"    • λ = 1/τ              : {lam_s_str}   ({lam_ms_str})\n"
            f"    • τ𝚠 (weighted)         : {tau_w:.2f}\n"
            f"    • λ𝚠 (weighted)       : {lam_w:.3f}\n"
            f"    • Decay 90→10 %        : {r['decay_time']:.2f}\n"
            f"    • R² (fit)             : {r['r2']:.3f}\n"
            f"    • A (mV)               : {A_str}\n"
            f"    • C (mV)               : {Cval:.2f}\n"))
